...
const loginUser = asyncHandler(async(req,res)=>{
    try {
        ...
        const statusCode = error.status || 500;
        return res.status(statusCode).json(new ApiError(statusCode,error.message || "Error while logging in the  User"))
    }
})
const getCurrentUser = asyncHandler ( async(req,res)=>{
    try {
        ...
        const statusCode = error.status || 500;
        return res.status(statusCode).json(new ApiError(statusCode,error.message || "Error while fetching the user"));
    }
})
